/* Auth + Header UI + Booking Guard (frontend-only, no backend)

Behavior:
- Logged OUT: right corner shows [Login]
- Logged IN: right corner shows [Reserve] [Logout] + "Hi, name"
- Booking is protected: redirects to login if not logged in.

Forms auto-wiring:
- If a page has #registerForm → we handle registration + save to localStorage.
- If a page has #loginForm → we handle login and redirect.
*/
...
(function () {
  ...
  // --- Header UI: ensure/right box, render state ---
  ...
  function renderAuthUI() {
    ...
    if (u) {
      ...
      who.textContent = 'Hi, ' + (u.name || u.email);
      who.style.color = '#cbd3d9';

      const reserve = document.createElement('button');
      reserve.className = 'cta';
      reserve.textContent = 'Reserve';
      reserve.onclick = () => (location.href = 'booking.html');

      const logout = document.createElement('button');
      logout.className = 'cta-outline';
      ...
      loginBtn.className = 'cta-outline';
  ...
  // --- Optional: booking handler with Luhn check ---
  function luhnOk(num) {
  ...
  window.handleBooking = function (e) {
    ...
    const all = safeParse(localStorage.getItem('az_bookings'), []);
    all.push(payload);
  ...
  // --- Auto-bind forms if present ---
  function bindFormsIfPresent() {
    // Register form: #registerForm, fields: r_name, r_email, r_password, r_confirm
    ...
    // Login form: #loginForm, fields: email, password
  ...
  // --- Image auto-fix helpers (handle case/extension differences on GitHub Pages)
...
function initCarImagesFallback() {
  document.querySelectorAll('img[data-candidates]').forEach(imgEl => {
    ...
    tryImagesSequentially(candidates, (ok) => {
      ...
      // last resort: show a simple placeholder if nothing matched
...
// Run after your existing boot code
document.addEventListener('DOMContentLoaded', () => {
  // (your existing boot code...)
  initHeroImageFallback();
  initCarImagesFallback();
});
